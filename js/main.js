...
for (let produto of produtos.products) {
    let image = produto.image || 'http://dummyimage.com/470x594/fff/000?text=imagem+indisponível'
    let result = `<li class="product-item">
    ...
    <div class="product-price">
      <p class="product-cost">
        <span class="actual-price">${produto.actual_price}</span>
        <span class="installments">${produto.installments}</span>
      </p>
      <p class="product-sale">
        ${produto.on_sale ? ` <del class="regular-price">${produto.regular_price}</del>
        <span class="stamp">(${produto.discount_percentage} OFF)</span>` : ''}
      </p>
    </div>
    <ul class="product-sizes">
        ${produto.sizes
            .filter(s => s.available)
            .map(a => `<li><a href="#" class="size ${a.available ? 'available' : ''}">${a.size}</a></li>`).join('')}
    </ul>
  </li>`
  catalog.insertAdjacentHTML('beforeend', result)
}
...
off.addEventListener('change', () => {
  if (off.checked) {
    let result = produtos.products.filter(p => p.on_sale)
    catalog.innerHTML = result.map(generateCatalog).join('')
    total.innerHTML = `${result.length} itens`
  }
  else { 
    let result = produtos.products.map(e => e)
    catalog.innerHTML = result.map(generateCatalog).join('')
    total.innerHTML = `${result.length} itens`
  }
})
...
function generateCatalog(product) {
  let image = product.image || 'http://dummyimage.com/470x594/fff/000?text=imagem+indisponível'
  ...
  <div class="product-price">
    ...
    <p class="product-sale">
      ${product.on_sale ? ` <del class="regular-price">${product.regular_price}</del>
